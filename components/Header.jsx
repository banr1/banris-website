...
const HeaderName = forwardRef(function HeaderName({ siteTitle, postTitle, onClick }, ref) {
  return (
    <p
      ref={ref}
      className='header-name ml-2 font-medium text-lg text-gray-700 dark:text-gray-300 capture-pointer-events grid-rows-1 grid-cols-1 items-center'
      onClick={onClick}
    >
...
export default function Header({ navBarTitle, isFullWidth }) {
  ...
  // Favicon
  const resolveFavicon = fallback => !fallback && '/favicon_big.png';
    ...
    <>
      ...
      >
        ...
        <Link href={`${prefix}/`} aria-label={BLOG.title} className='flex items-center'>
          <Image src={favicon} width={30} height={30} alt='terminal_icon' onError={() => setFavicon(true)} />
